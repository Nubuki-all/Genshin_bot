...
async def fetch_artifact_detail(artifact: dict) -> tuple:
    name = artifact.get("name")
    rarity_list = artifact.get("rarityList")
    ...
    caption = f"*{name}*\n"
    caption += f"{'⭐' * max_rarity}\n\n"
    caption += f"*2-Pc:* {eff_two_pc}\n"
    caption += f"*4-Pc:* {eff_four_pc}"
...
async def generate_artifact_image(images, rarity, name, background_size=(800, 600)):
    ...
    :param background_size: Tuple (width, height) for background (default: 800x600)
    """
